...
function loadHtml5LightBox(jsFolder) { (function($) {
        $.fn.html5lightbox = function(options) {
            ...
            inst.checkParentClass = function(node, parentNode, attr) {
                var ret = null;
                $.each([node, parentNode],
                function(index, nodeValue) {
            ...
            inst.readData = function() {
                inst.each(function() {
                    var self = $(this);
                    if (this.nodeName.toLowerCase() == "a" || this.nodeName.toLowerCase() == "area" || this.nodeName.toLowerCase() == "input") inst.readNodeData(self);
            ...
            inst.showTitle = function(t, description) {
                ...
                if (inst.options.showsocialmedia) if (inst.currentElem[ELEM_SOCIALMEDIA]) if ($("#html5-socialmedia", inst.$lightboxBox).length > 0) $("#html5-socialmedia", inst.$lightboxBox).html(inst.currentElem[ELEM_SOCIALMEDIA]);
            ...
            inst.showYoutubeVimeo = function(elem) {
                ...
                } else inst.resizeLightbox(dataW, dataH, true,
                function() {
                    inst.showYouTubeVimeoCallback(elem, sizeObj);
                    inst.showData()
                })
            ...
            inst.showPDF = function(elem) {
                if (inst.options.enablepdfjs) {
                    ...
                    var pdfelem = jQuery.extend(true, {},
                    elem);
            ...
            inst.showWeb = function(elem) {
                ...
                function() {
                    ...
                    $(".html5lightbox-web-iframe", inst.$image).on("load",
                    function() {
                        inst.$loading.hide()
                    });
                    inst.$elem.show();
            ...
            inst.resizeLightbox = function(elemW, elemH, bAnimate, onFinish) {
                ...
                if (!inst.options.fullscreenmode && (!inst.options.isTouch || !inst.options.navarrowsalwaysshowontouch) && !inst.options.alwaysshownavarrows) {
                    ...
                    inst.$lightboxBox.on("mouseleave",
                    function() {
                        inst.$next.fadeOut();
                        inst.$prev.fadeOut()
                    })
            ...
            inst.finish = function() {
                if (inst.options.exitanimation == "fadeOut" || inst.options.exitanimation == "fadeOutDown") {
                    inst.$lightbox.one("webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend",
                    function() {
            ...
            inst.embedHTML5Video = function($container, src, autoplay, loopvideo, mutevideo) {
                ...
                $("video", $container).off("ended").on("ended",
                function() {
                    $(window).trigger("html5lightbox.videofinished");
                    if (inst.autosliding) inst.gotoSlide( - 1);
                    else if (inst.options.autoclose) setTimeout(function() {
            ...
            inst.each(function() {
                ...
                if (self.data("preloadonpageload")) {
                    ...
                    if (fileType == 0) {
                        ...
                        setTimeout(function() { (new Image).src = self.attr("href")
                        },
                        preloaddelay)
                    }
                }
            ...
            if (inst.options.preloadallonpageload) setTimeout(function() {
                inst.each(function() {
                    ...
                    if (fileType !== 0) return; (new Image).src = $(this).attr("href");
